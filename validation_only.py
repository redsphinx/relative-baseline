...
from model_0 import Siamese as Siamese_pilot
...
# predict_valence_sequential_relative('val', 10, 99, time_gap=1, model_number=4, label_type='discrete')
...
def predict_valence_sequential_single(which, experiment_number, epoch):
    ...
    print('model_%d_experiment_%d_epoch_%d, val_loss: %f, CCC: %f, pearson: %f' % (model_number, experiment_number, epoch,
                                                                          float(np.mean(_loss_steps)),
                                                                          float(np.mean(_all_ccc)),
                                                                                       float(np.mean(_all_pearson))))


# predict_valence_sequential_single('val', 6, 89)
...
# def predict_value_model_0('val', 15, 89, time_gap=1, model_number=0, label_type='discrete'):
def predict_value_model_0(experiment_number, epoch, which='val', time_gap=1, model_number=0):
    use_ccc = True
    use_pearson = True
    _loss_steps = []
    _all_ccc = []
    ...
    print('model_%d_experiment_%d_epoch_%d, val_loss: %f, CCC: %f, pearson: %f' % (model_number,
                                                                                   experiment_number,
                                                                                   epoch,
                                                                                   float(np.mean(_loss_steps)),
                                                                                   float(np.mean(_all_ccc)),
...
# predict_value_model_0(experiment_number=15, epoch=89)
