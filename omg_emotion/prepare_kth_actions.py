...
def prepare_data_60_60():
    def avi_to_png(vid_path, img_path):
        command = 'ffmpeg -loglevel panic -i ' + vid_path + ' -f image2 -vf "scale=80:60, crop=60:60:10:0" ' + img_path + '/%d.png'
        subprocess.call(command, shell=True)

    def scale_png(img_path):
        command = "ffmpeg -y -i %s -vf scale=80:60 %s" % (img_path, img_path)
        subprocess.call(command, shell=True)

    def crop_png(img_path):
        command = 'ffmpeg -y -i %s -vf "crop=60:60:10:0" %s' % (img_path, img_path)
        subprocess.call(command, shell=True)
    ...
    for c in classes:
        ...
        for avi in tqdm.tqdm(all_avis):
            ...
            if person in train:
                ...
                which = 'test'
            save_path = os.path.join(PP.kth_png_60_60, which, c, avi.split('_uncomp')[0])
            if not os.path.exists(save_path):
                os.makedirs(save_path)
            avi_to_png(avi_path, save_path)

            all_pngs = os.listdir(save_path)
