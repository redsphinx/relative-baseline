...
class LeNet5_TTN3d(torch.nn.Module):
    ...
    def forward(self, x):
        # convolve, then perform ReLU non-linearity
        # TODO: batch of 20???
        x = torch.nn.functional.relu(self.conv1(x, stride=(1, 1, 1), padding=(2, 2, 2), dilation=(1, 1, 1), groups=1))
        # max-pooling with 2x2 grid
        x = self.max_pool_1(x)
        # convolve, then perform ReLU non-linearity
        x = torch.nn.functional.relu(self.conv2(x, stride=(1, 1, 1), padding=(0, 0, 0), dilation=(1, 1, 1), groups=1))
        # max-pooling with 2x2 grid
        x = self.max_pool_2(x)
        # first flatten 'max_pool_2_out' to contain 16*5*5 columns
        # read through https://stackoverflow.com/a/42482819/7551231
